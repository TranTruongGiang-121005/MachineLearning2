...
OUTPUT_FILE = './results/features_enhanced_all.csv'
...
def process_all():
    print(f"Scanning directory: {GENRES_DIR}")
    
    if not os.path.exists(GENRES_DIR):
        print("ERROR: Genre directory not found!")
        return
            ...
            if file.endswith('.wav'):
                ...
                try:
                    ...
                    for i in range(num_chunks):
                        ...
                        #add metadata
                        row['label'] = genre
                        row['filename'] = file # Crucial for GroupShuffleSplit later
                        data.append(row)
